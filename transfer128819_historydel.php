<?php
	...
	if(ISSET($_REQUEST['id'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `128819_history` WHERE `id`='$id'") or die(mysqli_error());
	...
	//	mysqli_query($conn, "INSERT INTO `filesscnhs_history` (id,name,date_updated)VALUES('$id','$name', '$date_updated')") or die(mysqli_error());
		...
		mysqli_query($conn, "DELETE FROM `128819_history` WHERE `id`='$id'") or die(mysqli_error());
		
		// echo"<script>alert('Data successfully Deleted')</script>";
		echo"<script>window.location='128819_history.php'</script>";
	}
