...
if(isset($_POST['search']))
{
    ...
    $query = "SELECT * FROM `filessnhs` WHERE  CONCAT(`id`, `name`,`date_updated`)LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);
    
}
    else {
        $query ="SELECT * FROM `filessnhs`ORDER BY id DESC";
        $search_result = filterTable($query);
...
<nav class="navbar navbar-inverse navbar-fixed-top">
  ...
  <a href="filessnhs_history.php"> <i class="material-icons" style="font-size:36px;color:Blue">cloud_queue</i><font color="Black"><B>Drive</B></font></a> 
      ...
      <a href="chat_snhs.php"><font color="Black"><B>Chat Room</a> | </B></font>
...
<div class="row">
         ...
         <form action="filessnhs.php" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <a type="button" class="btn btn-success" href="transfersnhs_history.php?id=<?php echo $row['id']?>">Okay</a>
                </div>
            </div>
        </div>
    </div> -->
...
function drive(data_id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location = ("transfersnhs_history.php?id=" + data_id);        
        }
    })
}
</script>
